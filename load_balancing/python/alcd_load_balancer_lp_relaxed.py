...
class ALCDWrapper:
  ...
  def __decompress(self):
    ...
    for i in range(self.num_servers):
      ...
      nzvals = [x * self.constraint_scale for x in self.shard_sizes]
      # memory on server i <= max_memory
      Amat.setrow(row_offset, list(zip(nzidxs, nzvals)))
      bvec[row_offset] = self.max_memory * self.constraint_scale
      coo_data.extend(nzvals)
...
def balance_load_alcd(num_shards, num_servers, shard_loads, shard_memory_usages,
  ...
  only_inequalities = False
  lpobj = ALCDWrapper(shard_loads, shard_memory_usages, current_xs, num_servers, max_memory, only_inequalities)
  primal_args = lpobj.get_primal_alcd_format()
  dual_lpargs = lpobj.get_dual_alcd_format()
  load_end_t = time.time()
  num_vars = num_shards * num_servers
  warm_start_guess = np.zeros(num_vars * 2)
  if len(current_rs) > 0:
    for i in range(num_servers):
      for j in range(num_shards):
        warm_start_guess[i * num_shards + j] = current_rs[i][j]
        warm_start_guess[num_vars + i * num_shards + j] = current_xs[i][j]
